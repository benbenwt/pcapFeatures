package com.ti.utils.flow;

import com.ti.utils.csv.InsertCsvRow;
...
import static com.ti.utils.config.Utils.LINE_SEP;
...
public class FlowGenerator {
	...
	public void dealFlowOverTime(BasicFlow flow, BasicPacketInfo packet,String id,String savePath,String filename){
//		System.out.println("step into TimeOut branch,and currentFlows size is : "+currentFlows.size());
		if(flow.packetCount()>1){
			ExecutorService csvWriterThread  = Executors.newSingleThreadExecutor();;
			String header  = FlowFeature.getHeader();
//			csvWriterThread.execute(new InsertCsvRow(header, flow.dumpFlowBasedFeaturesEx(), savePath, filename+ FlowMgr.FLOW_SUFFIX));
			new Thread(new InsertCsvRow(header, flow.dumpFlowBasedFeaturesEx(), savePath, filename+ FlowMgr.FLOW_SUFFIX)).run();
		}
		...
		if(cfsize%50==0) {
//			System.out.println("Timeout current has flow "+cfsize);
		}
	}
	public void dealFlowHasFlagFIN(BasicFlow flow,BasicPacketInfo packet,String id,String savePath,String filename){
//		System.out.println("step into FlagFIN branch,and currentFlows size is : "+currentFlows.size());
...
//					System.out.println("Backwards flow closed due to FIN Flag");
					flow.updateActiveIdleTime(currentTimestamp,this.flowActivityTimeOut);
					flow.addPacket(packet);
					currentFlows.put(id,flow);
				}
	...
	public void dealFlowExistInCurrentFlows(BasicPacketInfo packet,String savePath,String filename){
		BasicFlow   flow;
		long        currentTimestamp = packet.getTimeStamp();
		String id;

//		System.out.println("step into containsthisKey branch,and currentFlows size is : "+currentFlows.size());
		...
		if((currentTimestamp -flow.getFlowStartTime())>flowTimeOut){
			dealFlowOverTime(flow,packet,id, savePath, filename);
		}else if(packet.hasFlagFIN()){
			dealFlowHasFlagFIN(flow,packet,id, savePath, filename);
		}else if(packet.hasFlagRST()){
//			System.out.println("step into hasFlagRST branch,and currentFlows size is : "+currentFlows.size());
			flow.addPacket(packet);
			ExecutorService csvWriterThread  = Executors.newSingleThreadExecutor();;
			String header  = FlowFeature.getHeader();
//			csvWriterThread.execute(new InsertCsvRow(header, flow.dumpFlowBasedFeaturesEx(), savePath, filename+ FlowMgr.FLOW_SUFFIX));
	...
	public void addPacketMy(BasicPacketInfo packet,String savePath,String filename){
//		System.out.println("----------------------------------------------------------------------------------");
//		System.out.println("step into  addPacketMy function,and currentFlows size is : "+currentFlows.size());

		if(packet == null) {
			return;
		}
		if(this.currentFlows.containsKey(packet.fwdFlowId())||this.currentFlows.containsKey(packet.bwdFlowId())){
			dealFlowExistInCurrentFlows(packet,savePath,filename);
		}else{
...
//		System.out.println("Let`s dumpLabeledCurrentFlow,and the currentFlows size is "+currentFlows.size());
...
//				System.out.println("myCount0 :"+myCount);
				myCount++;
				if(flow.packetCount()>1) {
//					System.out.println("flow.packetCount()>1");
//                if(flow.packetCount()>1) {
...
//				System.out.println("myCount1 :"+myCount);
            }
        } catch (IOException e) {
