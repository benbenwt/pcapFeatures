package com.ti.utils.packet;

import com.ti.utils.domain.Protocol;
import com.ti.utils.flow.BasicPacketInfo;
import com.ti.utils.flow.IdGenerator;
...
public class PacketReader {
	...
	private void config(String filename){
		...
		if (pcapReader == null) {
			System.out.println("pcapReader==null,"+errbuf);
			throw new NullPointerException("pcapReader==null");
//			System.exit(-1);
		}else{
			this.tcp = new Tcp();
			this.udp = new Udp();
	...
	public BasicPacketInfo nextPacket(){
		 ...
		 try{
			 ...
			 System.out.println("Read All packets on {}"+file);
			 throw e;
		 }catch(Exception ex){
		...
		try {
			...
			if (packet.hasHeader(ipv4)){
				...
				if(packet.hasHeader(this.tcp)){
					...
					/*System.out.println("other packet Ethernet -> {}"+  packet.hasHeader(new Ethernet()));
					System.out.println("other packet Html     -> {}"+  packet.hasHeader(new Html()));
					System.out.println("other packet Http     -> {}"+  packet.hasHeader(new Http()));
					System.out.println("other packet SLL      -> {}"+  packet.hasHeader(new SLL()));
					System.out.println("other packet L2TP     -> {}"+  packet.hasHeader(new L2TP()));
					System.out.println("other packet Sctp     -> {}"+  packet.hasHeader(new Sctp()));
					System.out.println("other packet PPP      -> {}"+  packet.hasHeader(new PPP()));*/
					...
					for(int i=0;i<headerCount;i++) {
						...
						//System.out.println("getIpv4Info: {} --description: {} ",header.getName(),header.getDescription());
					}
				}
			}
		} catch (Exception e) {
